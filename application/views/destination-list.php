    ...
    <div class="container no-h-padding">
        <div class="isotope-wrap masonry-grid">
            <?php foreach ($destinations as $k =>  $destination): ?>
                <div class="element-item masonry-item col-md-4 col-sm-6 col-xs-12 with-space page-<?= floor(($k) / $limit) ?>">
                    <div class="portfolio-item">
                        <figure class="portfolio-figure">
                            <img src="<?= UP.$destination->Image ?>" alt="Portfolio Image">
                            <figcaption>
                                <div class="valign-wrap fullwidth fullheight">
                                    <div class="caption-wrap valign-middle">
                                        <h2><?= $destination->DestinationTitle ?></h2>
                                        <div class="separator"></div>
                                        <p><?= character_limiter($destination->ShortDescription,200) ?></p>
                                        <a href="<?=  base_url('destination/'.url_title($destination->DestinationTitle)."/$destination->DestinationId") ?>"
...
<!-- Portfolio Filter Begin -->
<section class="wow animated fadeIn text-center sec-pad-t">
    <div class="container">
        <div class="row">
            ...
            <div class="pagination-content">
                <div class="pagination-button">
                    <ul class="pagination">
                        <?php for ($i=1; $count+1 > $i ; $i++ ): ?>
                            <li <?= $page == $i ? "class='active'":"" ?> ><a href="<?= current_url()."/?page_id=$i" ?>"><?=$i?></a></li>
                        <?php endfor ?>
                    </ul>
                </div>
            </div>
